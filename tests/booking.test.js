...
describe('Tests des Réservations (Bookings) et de la Sécurité', () => {
  ...
  beforeEach(async () => {
    ...
    agencyParis = await Agency.create({ name: 'Autoclean Paris', address: { street: '1 rue', city: 'Paris', zipCode: '75000' }, phone: '0100000000' });
    agencyLyon = await Agency.create({ name: 'Autoclean Lyon', address: { street: '2 rue', city: 'Lyon', zipCode: '69000' }, phone: '0400000000' });

    // 3. Utilisateurs et Tokens
    const adminUser = await User.create({ firstName: 'Boss', lastName: 'Auto', email: 'admin@test.com', password: 'hash', phone: '0600000000', role: adminRole._id });
    adminToken = jwt.sign({ userId: adminUser._id, role: 'SuperAdmin' }, process.env.JWT_SECRET);

    const customerUser = await User.create({ firstName: 'Client', lastName: 'Zero', email: 'client@test.com', password: 'hash', phone: '0611111111', role: customerRole._id });
    customerToken = jwt.sign({ userId: customerUser._id, role: 'Customer' }, process.env.JWT_SECRET);

    const managerParis = await User.create({ firstName: 'Manager', lastName: 'Paris', email: 'manager@paris.com', password: 'hash', phone: '0622222222', role: managerRole._id, agency: agencyParis._id });
    managerParisToken = jwt.sign({ userId: managerParis._id, role: 'Manager' }, process.env.JWT_SECRET);
    ...
    bookingParis = await Booking.create({
      customer: customerUser._id, agency: agencyParis._id, service: testService._id,
      date: futureDate, totalPrice: 25, totalDurationMinutes: 30, status: 'Pending'
    });

    bookingLyon = await Booking.create({
      customer: customerUser._id, agency: agencyLyon._id, service: testService._id,
      date: futureDate, totalPrice: 25, totalDurationMinutes: 30, status: 'Pending'
    });
  });
  ...
  describe('POST /api/bookings et GET /my-bookings', () => {
    it('devrait calculer le bon prix à la création', async () => {
      ...
      const res = await request(app).post('/api/bookings').set('Authorization', `Bearer ${customerToken}`).send({
        agencyId: agencyParis._id.toString(), serviceId: testService._id.toString(), optionIds: [testOption._id.toString()], date: futureDate.toISOString()
      });
    ...
    it('devrait ramener les réservations du client', async () => {
      const res = await request(app).get('/api/bookings/my-bookings').set('Authorization', `Bearer ${customerToken}`);
  ...
  describe('GET /api/bookings (Toutes les réservations)', () => {
    ...
    it('devrait filtrer les réservations pour un Manager (Test Cloisonnement)', async () => {
      const res = await request(app).get('/api/bookings').set('Authorization', `Bearer ${managerParisToken}`);
      expect(res.statusCode).toBe(200);
  ...
  describe('GET et PUT /api/bookings/:id (Une seule réservation)', () => {
    it('devrait permettre au Manager de Paris de modifier une réservation de Paris', async () => {
      const res = await request(app).put(`/api/bookings/${bookingParis._id}`).set('Authorization', `Bearer ${managerParisToken}`).send({
        status: 'Confirmed'
      });
      expect(res.statusCode).toBe(200);
    ...
    it('devrait BLOQUER le Manager de Paris s\'il essaie de voir la réservation de Lyon', async () => {
      const res = await request(app).get(`/api/bookings/${bookingLyon._id}`).set('Authorization', `Bearer ${managerParisToken}`);
  ...
  describe('DELETE /api/bookings/:id', () => {
    it('devrait interdire la suppression à un Manager', async () => {
      const res = await request(app).delete(`/api/bookings/${bookingParis._id}`).set('Authorization', `Bearer ${managerParisToken}`);
    ...
    it('devrait permettre la suppression par le SuperAdmin', async () => {
      const res = await request(app).delete(`/api/bookings/${bookingParis._id}`).set('Authorization', `Bearer ${adminToken}`);
      expect(res.statusCode).toBe(200);
